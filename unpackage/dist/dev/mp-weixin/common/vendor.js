...
(function(global2, factory) {
  ...
  function constructUserAgent(version2, os2, runtime, runtimeVersion) {
  ...
  class FetchHttpClient extends HttpClient {
    ...
    async send(request) {
      ...
      try {
        ...
        this._logger.log(exports2.LogLevel.Warning, `Error from HTTP request. ${e2}.`);
        throw e2;
      } finally {
        if (timeoutId) {
  ...
  class HubConnection {
    ...
    async _startWithStateTransitions() {
      ...
      try {
        ...
        this._connectionState = exports2.HubConnectionState.Disconnected;
        this._logger.log(exports2.LogLevel.Debug, `HubConnection failed to start successfully because of error '${e2}'.`);
    ...
    async _startInternal() {
      ...
      try {
        ...
        this._logger.log(exports2.LogLevel.Debug, `Hub handshake failed with error '${e2}' during start(). Stopping HubConnection.`);
        this._cleanupTimeout();
            ...
            case exports2.MessageType.Completion: {
              ...
              if (callback) {
                ...
                try {
                  ...
                  this._logger.log(exports2.LogLevel.Error, `Stream callback threw error: ${getErrorString(e2)}`);
                }
              }
              break;
            }
    ...
    async _invokeClientMethod(invocationMessage) {
      ...
      for (const m2 of methodsCopy) {
        try {
          ...
          exception = e2;
          this._logger.log(exports2.LogLevel.Error, `A callback for the method '${methodName}' threw error '${e2}'.`);
        }
      }
    ...
    _completeClose(error) {
      if (this._connectionStarted) {
        ...
        try {
          ...
          this._logger.log(exports2.LogLevel.Error, `An onclose callback called with error '${error}' threw error '${e2}'.`);
        }
      }
    }
    async _reconnect(error) {
      ...
      if (this._reconnectingCallbacks.length !== 0) {
        try {
          ...
          this._logger.log(exports2.LogLevel.Error, `An onreconnecting callback called with error '${error}' threw error '${e2}'.`);
        }
        ...
        try {
          ...
          if (this._reconnectedCallbacks.length !== 0) {
            try {
              ...
              this._logger.log(exports2.LogLevel.Error, `An onreconnected callback called with connectionId '${this.connection.connectionId}; threw error '${e2}'.`);
            }
          }
          return;
        } catch (e2) {
          this._logger.log(exports2.LogLevel.Information, `Reconnect attempt failed because of error '${e2}'.`);
          if (this._connectionState !== exports2.HubConnectionState.Reconnecting) {
    ...
    _getNextRetryDelay(previousRetryCount, elapsedMilliseconds, retryReason) {
      try {
        ...
        this._logger.log(exports2.LogLevel.Error, `IRetryPolicy.nextRetryDelayInMilliseconds(${previousRetryCount}, ${elapsedMilliseconds}) threw error '${e2}'.`);
        return null;
      }
    }
    _cancelCallbacksWithError(error) {
      ...
      Object.keys(callbacks).forEach((key) => {
        ...
        try {
          ...
          this._logger.log(exports2.LogLevel.Error, `Stream 'error' callback called with '${error}' threw error: ${getErrorString(e2)}`);
        }
      });
    }
    _cleanupPingTimer() {
      ...
      try {
        while (this._running) {
          try {
            ...
            if (!this._running) {
              this._logger.log(exports2.LogLevel.Trace, `(LongPolling transport) Poll errored after shutdown: ${e2.message}`);
            } else {
      ...
      return new Promise((resolve2, reject) => {
        ...
        try {
          eventSource.onmessage = (e2) => {
            if (this.onreceive) {
              try {
                this._logger.log(exports2.LogLevel.Trace, `(SSE transport) data received. ${getDataDetail(e2.data, this._options.logMessageContent)}.`);
  ...
  class HttpConnection {
    ...
    async _stopInternal(error) {
      ...
      if (this.transport) {
        try {
          ...
          this._logger.log(exports2.LogLevel.Error, `HttpConnection.transport.stop() threw error '${e2}'.`);
    ...
    async _startInternal(transferFormat) {
      ...
      try {
        ...
        this._logger.log(exports2.LogLevel.Error, "Failed to start the connection: " + e2);
        this._connectionState = "Disconnected";
        this.transport = void 0;
        this._stopPromiseResolver();
        return Promise.reject(e2);
    ...
    _resolveTransportOrError(endpoint, requestedTransport, requestedTransferFormat) {
      ...
      if (transport === null || transport === void 0) {
        ...
        if (transportMatches(requestedTransport, transport)) {
          const transferFormats = endpoint.transferFormats.map((s2) => exports2.TransferFormat[s2]);
    ...
    _stopConnection(error) {
      ...
      if (this._sendQueue) {
        this._sendQueue.stop().catch((e2) => {
          this._logger.log(exports2.LogLevel.Error, `TransportSendQueue.stop() threw error '${e2}'.`);
        });
      ...
      if (this._connectionStarted) {
        ...
        try {
          ...
          this._logger.log(exports2.LogLevel.Error, `HttpConnection.onclose(${error}) threw error '${e2}'.`);
        }
      }
    }
    _resolveUrl(url) {
    ...
    _resolveNegotiateUrl(url) {
      ...
      let negotiateUrl = url.substring(0, index2 === -1 ? url.length : index2);
      if (negotiateUrl[negotiateUrl.length - 1] !== "/") {
        negotiateUrl += "/";
      }
      negotiateUrl += "negotiate";
      negotiateUrl += index2 === -1 ? "" : url.substring(index2);
