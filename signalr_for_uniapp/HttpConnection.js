...
import { SocketTaskTransport } from "./WebSocketTransport";
...
export class HttpConnection {
    constructor(url, options = {}) {
        ...
        if (!Platform.isNode && typeof SocketTaskConstructor !== "undefined" && !options.SocketTask) {
            options.SocketTask = SocketTaskConstructor;
        }
        else if (Platform.isNode && !options.SocketTask) {
            if (socketTaskModule) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (this._options.skipNegotiation) {
                    if (this._options.transport === HttpTransportType.SocketTasks) {
                        // No need to add a connection ID in this case
                        this.transport = this._constructTransport(HttpTransportType.SocketTasks);
    ...
    _constructTransport(transport) {
        switch (transport) {
            case HttpTransportType.SocketTasks:
                if (!this._options.SocketTask) {
                    throw new Error("'SocketTask' is not supported in your environment.");
                }
                return new SocketTaskTransport(this._httpClient, this._accessTokenFactory, this._logger, this._options.logMessageContent, this._options.SocketTask, this._options.headers || {});
        ...
        if (transport === null || transport === undefined) {
            ...
            if (transportMatches(requestedTransport, transport)) {
                ...
                if (transferFormats.indexOf(requestedTransferFormat) >= 0) {
                    if ((transport === HttpTransportType.SocketTasks && !this._options.SocketTask) ||
